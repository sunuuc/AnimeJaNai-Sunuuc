...
var player_ui=ScopedCommandLine.Parse(new[]{"--script=loader.lua","--{","--script-opt=handoff-url=http://127.0.0.1/video,handoff-tag=ready","--}"});
Check(player_ui.HasGroups && player_ui.Entries.Count==0,"empty Player UI scope has no fake media entry");
Check(player_ui.EmptyGroupCount==1 && player_ui.EmptyGroupOptionsPromoted==1,"empty Player UI scope option promoted");
Check(player_ui.PromotedOptions.Count==1 && player_ui.PromotedOptions[0].Name=="script-opt","promoted option provenance retained");
Check(player_ui.GlobalOptions.Count==2 && player_ui.GlobalOptions[1].Name=="script-opt","promoted option order and spelling retained");
Check(player_ui.LegacyScriptHandoff,"legacy script handoff detected");
Check(player_ui.NeedsDedicatedProcess,"legacy handoff stays in its own process");
var player_uiPlaylist=ScopedCommandLine.Parse(new[]{"--script=loader.lua","--{","--playlist=https://example.invalid/episodes.m3u","--playlist-start=24","--force-media-title=第 25 集","--}"});
Check(player_uiPlaylist.Entries.Count==0 && player_uiPlaylist.EmptyGroupOptionsPromoted==3,"Player UI playlist envelope promoted");
Check(player_uiPlaylist.PromotedOptions.Select(o=>o.Name).SequenceEqual(new[]{"playlist","playlist-start","force-media-title"}),"playlist option order retained");
Check(player_uiPlaylist.GlobalOptions.Count(o=>o.Name=="playlist")==1 && player_uiPlaylist.GlobalOptions.Single(o=>o.Name=="playlist-start").Value=="24","selected playlist index retained");
Check(player_uiPlaylist.LegacyScriptHandoff && player_uiPlaylist.NeedsDedicatedProcess,"playlist handoff stays dedicated");
